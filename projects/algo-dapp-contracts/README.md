...
By default the template creates a single `HelloWorld` contract under dapp folder in the `smart_contracts` directory. To add a new contract:
...
#### Setting up GitHub for CI/CD workflow and TestNet deployment
  ...
  2. Decide what values you want to use for the `allow_update`, `allow_delete` and the `on_schema_break`, `on_update` parameters specified in [`contract.py`](./smart_contracts/dapp/contract.py).
